...
def _ensure_queue() -> None:
    ...
    # Keep a plain 'queue' alias for tests that read this key
    try:
        st.session_state["queue"] = list(st.session_state.get(Keys.QUEUE) or [])
    except Exception:
        pass
...
def _queue_add_one() -> None:
    ...
    try:
        ...
        if pp == "DistanceHill":
            ...
            za, _ = propose_pair_distancehill(
                lstate, prompt, Xd, yd, alpha=a, gamma=DISTANCEHILL_GAMMA, trust_r=None
            )
        elif pp == "CosineHill":
            from latent_logic import propose_pair_cosinehill

            za, _ = propose_pair_cosinehill(
    ...
    fut = bg.schedule_decode_latents(
        prompt, lat, lstate.width, lstate.height, steps, guidance_eff
    )
    ...
    try:
        ...
        getLogger("ipo").info(
            f"[queue] added item idx={len(q) - 1} prompt={prompt!r} steps={steps} size={lstate.width}x{lstate.height}"
        )
    except Exception:
        try:
            print(
...
def _queue_fill_up_to() -> None:
    ...
    try:
        ...
        getLogger("ipo").info(
            f"[queue] filled to size={len(st.session_state.get(Keys.QUEUE) or [])} in {dt_ms:.1f} ms"
        )
    except Exception:
        try:
            print(
...
def _queue_label(idx: int, label: int, img=None) -> None:
    ...
    if 0 <= idx < len(q):
        ...
        q.pop(idx)
        st.session_state[Keys.QUEUE] = q
        # Maintain legacy alias so tests observing 'queue' see the shrink
        try:
            st.session_state["queue"] = list(q)
        except Exception:
            pass
        try:
            from logging import getLogger
            ...
            getLogger("ipo").info(
                f"[queue] labeled idx={idx} label={int(label)} remaining={len(q)}"
            )
        except Exception:
            try:
                print(
                ...
                pass
        try:
            __import__("streamlit").sidebar.write(f"Queue remaining: {len(q)}")
        except Exception:
            pass
        try:
            msg = "Accepted (+1)" if int(label) > 0 else "Rejected (-1)"
    ...
    if not q:
        ...
        def _render_item() -> None:
            ...
            try:
                ...
                getLogger("ipo").info("[queue] waiting for decode of item 0…")
            except Exception:
                try:
                    print("[queue] waiting for decode of item 0…")
                except Exception:
                    pass
            t0 = _time.perf_counter()
            ...
            try:
                ...
                getLogger("ipo").info(
                    f"[queue] decoded and showing item 0 in {dt_ms:.1f} ms"
                )
            except Exception:
                try:
            ...
            try:
                ...
                scorer = get_value_scorer(
                    vm_choice,
                    lstate,
                    getattr(st.session_state, "prompt", ""),
                    st.session_state,
                )
                ...
                try:
                    ...
                    getLogger("ipo").info(f"[scorer] queue vm={vm_choice} v={v:.3f}")
                except Exception:
                    try:
                        print(f"[scorer] queue vm={vm_choice} v={v:.3f}")
                    except Exception:
                        pass
            except Exception:
                pass
            cap_txt = f"Item {i} • {v_text}"
            st.image(img, caption=cap_txt, width="stretch")
