    ...
    def test_std_tracks_scheduler_init_sigma_after_set_timesteps(self):
        ...
        class _Sched:
            def __init__(self):
                ...
                self.sigmas = types.SimpleNamespace(
                    max=lambda: types.SimpleNamespace(
                        item=lambda: self.init_noise_sigma
                    )
                )
        ...
        img = flux_local.generate_flux_image_latents(
            "p", latents=lat, width=64, height=64, steps=6, guidance=2.0
        )
