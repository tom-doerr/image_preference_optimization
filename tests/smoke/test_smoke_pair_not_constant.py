    ...
    def test_pair_images_have_content(self):
        ...
        A = self._to_np(
            generate_flux_image_latents(
                prompt, latents=lat_a, width=512, height=512, steps=6, guidance=2.5
            )
        )
        B = self._to_np(
            generate_flux_image_latents(
                prompt, latents=lat_b, width=512, height=512, steps=6, guidance=2.5
            )
        )
        ...
        self.assertGreater(
            float(
                np.abs(np.asarray(A, dtype=float) - np.asarray(B, dtype=float)).std()
            ),
            1.0,
        )
