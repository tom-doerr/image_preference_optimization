...
class TestPairNotConstantStub(unittest.TestCase):
    ...
    def test_pair_images_have_variance_stub(self):
        ...
        A = flux_local.generate_flux_image_latents(prompt, la, width=320, height=256, steps=4, guidance=2.0)
        B = flux_local.generate_flux_image_latents(prompt, lb, width=320, height=256, steps=4, guidance=2.0)
        An = np.asarray(A)
        Bn = np.asarray(B)
        self.assertGreater(float(An.std()), 1.0)
        self.assertGreater(float(Bn.std()), 1.0)
        self.assertGreater(float(np.abs(An.astype(float) - Bn.astype(float)).std()), 0.5)
