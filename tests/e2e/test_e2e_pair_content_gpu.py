...
class TestE2EPairContentGPU(unittest.TestCase):
    ...
    def test_pair_images_have_content(self):
        ...
        model_id = os.getenv(
            "SMOKE_MODEL", os.getenv("FLUX_LOCAL_MODEL", "stabilityai/sd-turbo")
        )
        set_model(model_id)
        ...
        A = self._to_np(
            generate_flux_image_latents(
                prompt, latents=la, width=512, height=512, steps=6, guidance=2.5
            )
        )
        B = self._to_np(
            generate_flux_image_latents(
                prompt, latents=lb, width=512, height=512, steps=6, guidance=2.5
            )
        )
