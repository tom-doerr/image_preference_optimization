...
class BatchKeysUniqueTest(unittest.TestCase):
    ...
    def test_keys_change_each_render(self):
        ...
        # Capture button keys passed
        pressed = []

        def button(label, *a, **k):
            pressed.append(k.get("key"))
            return False
        ...
        st.session_state.dataset_X = None
        st.session_state.dataset_y = None
...
import batch_ui
        ...
        self.assertGreater(len(set(k for k in pressed if k)), len(pressed) / 2)
