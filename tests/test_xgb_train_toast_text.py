...
class TestXgbTrainToastText(unittest.TestCase):
    def test_toast_after_sync_fit(self):
        ...
        # Minimal dataset via _get_dataset_for_display: return 2x rows and both classes
        import numpy as np
        X = np.ones((4, 3))
        y = np.array([+1, -1, +1, -1])
        ...
        self.assertEqual(getattr(st.session_state, '_last_toast', ''), "XGBoost: trained (sync)")
