...
def get_dataset_for_prompt_or_session(prompt, ss):
    ...
    if xs:
        ...
        print(f"[data] loaded {X.shape[0]} from {root}")
        return X, y
    return None, None
...
def save_sample_image(prompt, row_idx, img):
    try:
        ...
        d = os.path.join(data_root_for_prompt(prompt), f"{row_idx:06d}")
        os.makedirs(d, exist_ok=True)
        with _mem_lock(prompt), _file_lock(prompt):
            pil_img = img if hasattr(img, "save") else Image.fromarray(np.asarray(img))
            pil_img.save(os.path.join(d, "image.png"))
    except (OSError, ValueError, TypeError):
        pass  # non-critical: skip image save on error


def append_sample(prompt, feat, label, img=None):
    idx = append_dataset_row(prompt, feat, float(label))
    if img is not None:
        save_sample_image(prompt, idx, img)
    return idx
...
def export_state_bytes(state, prompt):
    ...
    with np.load(io.BytesIO(raw)) as d:
        items = {k: d[k] for k in d.files}
    items.update(
        prompt=np.array(prompt),
        created_at=np.array(datetime.now(timezone.utc).isoformat()),
        app_version=np.array(APP_VERSION)
    )
    buf = io.BytesIO()
